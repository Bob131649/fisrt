...
import copy
from networks.net import Actor, Critic, ActorVAE
...
class Latent(nn.Module):
    def __init__(self, state_dim, action_dim, latent_dim, min_v, max_v, 
        ...
        self.g_clip = 0.5
        self.q_buffer = deque(maxlen=200)
    ...
    def explore(self, state, need_q=False):
        with torch.no_grad():
            state = torch.FloatTensor(state.reshape(1, -1)).to(self.device)
            # latent_a = self.actor(state)
            # latent_a += torch.randn_like(latent_a) * 0.3
            latent_a = None
    ...
    def train_step(self, batch, iter_id, r_shift, r_scale):
        state, action, next_state, reward, not_done = batch
        reward = (reward-r_shift)/r_scale
        ...
        if iter_id % 1 == 0:
            with torch.no_grad():
                ...
                # next_q = self.get_pi_q(next_state, self.actor, self.critic, self.actor_vae,
                #                        type='min', use_noise=False) #.clamp(self.min_v, np.inf)
                # q_a = reward + not_done * self.discount * next_q
            ...
            # actor_vae_loss = actor_vae_loss.sum() / weight.sum()
        ...
        if iter_id % 5 == 0:
            ...
            actor_loss = -q_pi.mean() + lambda_reg * dist_norm
        ...
        return w_check.sum().cpu().numpy(), a_loss, critic_loss.item(), loss_rc, loss_kl
