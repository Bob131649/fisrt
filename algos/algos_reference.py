...
class Latent(nn.Module):
    def __init__(self, state_dim, action_dim, latent_dim, min_v, max_v, 
        ...
        self.vnet_optimizer = torch.optim.Adam(self.vnet.parameters(), lr=critic_lr, weight_decay=1e-3)
    ...
    def get_pi_q(self, state, q_net, gen_net, type='default', use_noise=True):
        ...
        if type == 'default':
            ...
            # q_min = torch.min(target_q1, target_q2)
            # # q_max = torch.max(target_q1, target_q2)
            # q_mean = (target_q1 + target_q2) / 2

            # gap_cap = 0.05 * torch.abs(q_mean).clamp(min=5.0)
            # floor = q_mean - gap_cap
            # target_q = torch.maximum(q_min, floor)
    ...
    def train_q(self, batch, idx):
        ...
        # td_error2 = (current_q1 - current_q2).detach()
        # print(td_error2.abs().mean(), td_error2.abs().max(), current_q1.max(), current_q2.max(), target_q.max())
