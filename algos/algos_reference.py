...
class Latent(nn.Module):
    def __init__(self, state_dim, action_dim, latent_dim, min_v, max_v, 
        ...
        self.qnet_optimizer = torch.optim.Adam(self.qnet.parameters(), lr=critic_lr)

        self.vnet = Value(state_dim).to(self.device)
        self.vnet_optimizer = torch.optim.Adam(self.vnet.parameters(), lr=critic_lr)
    ...
    def train_value(self, state_random_inr, state_expert_inr, state_expert_ine, a_expert_ine):
        with torch.no_grad():
