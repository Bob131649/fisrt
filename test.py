...
import algos.algos_reference as algos
import gym, d4rl
...
def eval_value(policy, buffer_expert, buffer_random, sample_size=1500, mode='q'):
    state_random, action_random, _, _, _, _ = buffer_random.sample(sample_size)
    state_expert, action_expert, _, _, _, _ = buffer_expert.sample(sample_size)

    if mode == 'q':
        state_random = renorm_random_expert(state_random, buffer_expert, buffer_random)
        action_random = renorma_random_expert(action_expert, buffer_expert, buffer_random)
        value_random = policy.get_a_q(state_random, action_random)
        value_expert = policy.get_a_q(state_expert, action_expert)
    if mode == 'v':
...
if __name__ == "__main__":
    ...
    if 'antmaze' in args.env_name:
        # dataset_expert['rewards'] = (dataset_expert['rewards']*100)
        # dataset_random['rewards'] = (dataset_random['rewards']*100)
    ...
    if args.mode == 'v' or args.load_model != -1:
        ...
        for epoch_idx in tglobal:
            ...
            if args.mode == 'pi':
                with tqdm(range(args.eval_freq), desc='Batch', leave=False) as tepoch:
                    for idx in tepoch:
                        batch = replay_buffer_expert.sample(args.batch_size)
                ...
                print('done')
                policy.eval()
                info = eval_policy(policy, env, replay_buffer_expert, plot=args.plot)
                policy.train()
            ...
            if args.mode == 'v':
                ...
                with tqdm(range(args.eval_freq), desc='Batch', leave=False) as tepoch:
                    for idx in tepoch:
                        batch_random = replay_buffer_random.sample(args.batch_size)
                        batch_expert = replay_buffer_expert.sample(args.batch_size)
                        state_random_inr, _, _, _, _, _ = batch_random
                        state_expert_ine, a_expert_ine, _, _, _, _ = batch_expert

                        # state_random_ine = renorm_random_policy(state_random_inr, replay_buffer_expert, replay_buffer_random)
                        state_expert_inr = renorm_expert_random(state_expert_ine, replay_buffer_expert, replay_buffer_random)

                        v_loss, v_random, v_expert = policy.train_value(state_random_inr, state_expert_inr, state_expert_ine, a_expert_ine)
...
# python train_reference.py --ExpID 1 --env_name antmaze-large-diverse --plot --mode pi --device cuda:0
...
# python train_reference.py --ExpID 1 --env_name maze2d-large --plot --mode pi --device cuda:1
# python train_reference.py --ExpID 2 --env_name maze2d-large --plot --mode q --device cuda:1
# python train_reference.py --ExpID 3 --env_name maze2d-large --plot --mode v --device cuda:1 --load_model 2